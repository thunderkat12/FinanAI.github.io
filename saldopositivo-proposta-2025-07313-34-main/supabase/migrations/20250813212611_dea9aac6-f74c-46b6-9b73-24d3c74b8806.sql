-- Create a helper function to safely create dates with month limits
CREATE OR REPLACE FUNCTION safe_make_date(year_val INTEGER, month_val INTEGER, day_val INTEGER)
RETURNS DATE
LANGUAGE plpgsql
IMMUTABLE
AS $function$
BEGIN
  ...
  DECLARE
    last_day_of_month INTEGER := EXTRACT(day FROM (date_trunc('month', make_date(year_val, month_val, 1)) + interval '1 month - 1 day'));
  BEGIN
    -- Use the minimum between requested day and last day of month
    RETURN make_date(year_val, month_val, LEAST(day_val, last_day_of_month));
  END;
END;
$function$;

-- Update the auto_generate_credit_card_bills function to use safe date creation
...
BEGIN
  ...
  LOOP
    ...
    IF existing_bill_id IS NULL THEN
      -- Calculate bill dates using safe date creation
      bill_closing_date := safe_make_date(bill_year, bill_month, card_record.closing_day);
      bill_due_date := safe_make_date(bill_year, bill_month, card_record.due_day);
      ...
      IF card_record.due_day <= card_record.closing_day THEN
        IF bill_month = 12 THEN
          bill_due_date := safe_make_date(bill_year + 1, 1, card_record.due_day);
        ELSE
          bill_due_date := safe_make_date(bill_year, bill_month + 1, card_record.due_day);
        END IF;
      END IF;
