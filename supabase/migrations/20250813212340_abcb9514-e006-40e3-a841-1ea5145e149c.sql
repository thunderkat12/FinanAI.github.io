-- Fix date calculation issue in auto_generate_credit_card_bills function
...
BEGIN
  ...
  LOOP
    ...
    IF existing_bill_id IS NULL THEN
      -- Calculate bill dates with proper date validation
      -- Ensure closing day doesn't exceed the month's max days
      bill_closing_date := LEAST(
        make_date(bill_year, bill_month, EXTRACT(days FROM date_trunc('month', make_date(bill_year, bill_month, 1)) + interval '1 month - 1 day')::INTEGER),
        make_date(bill_year, bill_month, card_record.closing_day)
      );
      
      -- Calculate due date with proper validation
      bill_due_date := LEAST(
        make_date(bill_year, bill_month, EXTRACT(days FROM date_trunc('month', make_date(bill_year, bill_month, 1)) + interval '1 month - 1 day')::INTEGER),
        make_date(bill_year, bill_month, card_record.due_day)
      );
      ...
      IF card_record.due_day <= card_record.closing_day THEN
        IF bill_month = 12 THEN
          ...
          bill_due_date := LEAST(
            make_date(bill_year + 1, 1, EXTRACT(days FROM date_trunc('month', make_date(bill_year + 1, 1, 1)) + interval '1 month - 1 day')::INTEGER),
            make_date(bill_year + 1, 1, card_record.due_day)
          );
        ELSE
          -- Next month in same year
          bill_due_date := LEAST(
            make_date(bill_year, bill_month + 1, EXTRACT(days FROM date_trunc('month', make_date(bill_year, bill_month + 1, 1)) + interval '1 month - 1 day')::INTEGER),
            make_date(bill_year, bill_month + 1, card_record.due_day)
          );
        END IF;
      END IF;
